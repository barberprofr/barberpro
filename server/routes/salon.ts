...
import {
  ...
  Service, ProductType, Product, Settings, Acompte,
  IStylist, IClient, IPrestation, IPointsRedemption,
  IService, IProductType, IProduct, ISettings, IAcompte,
  PaymentMethod
} from "./models.ts";
...
export const listAcomptes: RequestHandler = async (req, res) => {
  try {
    const salonId = getSalonId(req);
    const { stylistId, startDate, endDate, validated } = req.query as {
      stylistId?: string;
      startDate?: string;
      endDate?: string;
      validated?: string;
    ...
    if (startDate || endDate) {
      filter.timestamp = {};
      if (startDate) filter.timestamp.$gte = parseInt(startDate);
      if (endDate) filter.timestamp.$lte = parseInt(endDate);
    }
    ...
    const acomptes = await Acompte.find(filter).sort({ timestamp: -1 }).lean();
    
    // Calculate totals
    const total = acomptes.reduce((sum, a) => sum + a.amount, 0);
    const totalValidated = acomptes.filter(a => a.validated).reduce((sum, a) => sum + a.amount, 0);
    const totalPending = acomptes.filter(a => !a.validated).reduce((sum, a) => sum + a.amount, 0);
    ...
    console.error('Error listing acomptes:', error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};

export const createAcompte: RequestHandler = async (req, res) => {
  try {
    const body = await parseRequestBody(req);
    const salonId = getSalonId(req);
    const { stylistId, amount, note } = body as {
      stylistId: string;
      amount: number;
      note?: string;
    };

    if (!stylistId || typeof amount !== 'number' || amount <= 0) {
      return res.status(400).json({ error: "stylistId et amount (> 0) sont requis" });
    }
    ...
    const id = `acompte-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const acompte = new Acompte({
      id,
      stylistId,
      amount,
      timestamp: Date.now(),
    ...
    await acompte.save();
    res.status(201).json({ acompte });
  } catch (error) {
    console.error('Error creating acompte:', error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};

export const validateAcompte: RequestHandler = async (req, res) => {
  try {
    const salonId = getSalonId(req);
    const { id } = req.params;

    const acompte = await Acompte.findOne({ id, salonId });
    if (!acompte) {
      return res.status(404).json({ error: "Acompte non trouvé" });
    }

    acompte.validated = true;
    acompte.validatedAt = Date.now();
    await acompte.save();

    res.json({ acompte });
  } catch (error) {
    console.error('Error validating acompte:', error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};

export const deleteAcompte: RequestHandler = async (req, res) => {
  try {
    const salonId = getSalonId(req);
    const { id } = req.params;

    const acompte = await Acompte.findOneAndDelete({ id, salonId });
    if (!acompte) {
      return res.status(404).json({ error: "Acompte non trouvé" });
    }
    ...
    console.error('Error deleting acompte:', error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};

export const getStylistAcompteSummary: RequestHandler = async (req, res) => {
  try {
    const salonId = getSalonId(req);
    ...
    const filter: any = { salonId, stylistId, validated: true };
    ...
    const acomptes = await Acompte.find(filter).sort({ timestamp: -1 }).lean();
    const totalValidated = acomptes.reduce((sum, a) => sum + a.amount, 0);
    ...
    console.error('Error getting stylist acompte summary:', error);
    res.status(500).json({ error: "Erreur serveur" });
  }
};