        ...
        <AnimatePresence>
          ...
          >
            <motion.div
              ...
              transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
              onClick={(e) => e.stopPropagation()}
              className="relative w-full max-w-sm max-h-[calc(100vh-32px)] overflow-y-auto rounded-3xl bg-gradient-to-br from-slate-900/98 via-violet-900/60 to-slate-800/98 border border-violet-500/30 shadow-[0_25px_80px_rgba(0,0,0,0.6),0_0_40px_rgba(139,92,246,0.2)] backdrop-blur-xl p-8"
            >
              <button
                type="button"
                onClick={() => setProduitsPopupOpen(false)}
                className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white transition hover:bg-white/20"
              >
                ✕
              </button>
              <div className="flex flex-col items-center text-center">
    ...
    return (
      ...
      >
        <div className="space-y-4">
          ...
          <StylistMonthly id={selectedStylist.id} commissionPct={((selectedStylist as any).commissionPct ?? config?.commissionDefault ?? 0)} stylistName={selectedStylist.name} hiddenMonths={(selectedStylist as any).hiddenMonths || []} hiddenPeriods={(selectedStylist as any).hiddenPeriods || []} />
        </div>
      </motion.div>
    );
  }
...
function StylistCard({ s, config, onClick }: { s: any, config: any, onClick: () => void }) {
  ...
  return (
    <button
      ...
      className="flex flex-col items-center justify-center rounded-2xl border-2 border-amber-500/50 bg-black/25 backdrop-blur-xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.5)] aspect-square transition-all duration-150 hover:scale-[1.03] hover:border-amber-400/70 hover:bg-black/35 hover:shadow-[0_0_20px_rgba(251,191,36,0.3)] active:scale-95 active:border-amber-400 active:shadow-[0_0_30px_rgba(251,191,36,0.8)] cursor-pointer select-none"
