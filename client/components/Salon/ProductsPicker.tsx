...
export default function ProductsPicker({ onProductSelect, onReset, externalOpen, onOpenChange, disabled }: ProductsPickerProps) {
  ...
  return createPortal(
    ...
    >
      <div 
        className="w-full max-w-[min(90vw,36rem)] rounded-2xl border border-white/25 bg-slate-900/95 shadow-[0_40px_95px_rgba(8,15,40,0.8)] backdrop-blur-md p-0 animate-in zoom-in-95 duration-200"
                        ...
                        {isSelected && selectedData && (
                          ...
                          >
                            ...
                            <motion.button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setAnimatingQtyId(product.id);
                                setTimeout(() => {
                                  setAnimatingQtyId(null);
                                  setCalculatorProductId(product.id);
                                }, 200);
                              }}
                              animate={animatingQtyId === product.id ? { scale: 1.15, boxShadow: "0 0 20px rgba(52,211,153,0.8)" } : { scale: 1, boxShadow: "0 0 0px rgba(52,211,153,0)" }}
