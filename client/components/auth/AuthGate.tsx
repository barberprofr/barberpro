...
function Login({ onSwitchSignup, onRecover }: { onSwitchSignup: () => void; onRecover: () => void }) {
  ...
  return (
    <Card className="border border-white/15 shadow-[0_24px_68px_rgba(15,23,42,0.55)] min-h-[380px] bg-white/12 text-gray-100 backdrop-blur-2xl">
      <CardHeader>
        <CardTitle className="text-center">Se connecter</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="relative">
          <label className="absolute -top-2.5 left-3 px-1 text-xs font-medium text-fuchsia-400 bg-transparent z-10">Email</label>
          <Input type="email" value={email} onChange={(e)=>{ setEmail(e.target.value); setLoginError(""); }} autoComplete="email" className="pt-6 pb-5 px-5 bg-gray-800/50 border-gray-600 rounded-2xl text-gray-100 placeholder:text-gray-400" />
        </div>
        <div className="relative">
          <label className="absolute -top-2.5 left-3 px-1 text-xs font-medium text-fuchsia-400 bg-transparent z-10">Mot de passe personnalisé</label>
          <PasswordInput value={pwd} onChange={(e)=>{ setPwd(e.target.value); setLoginError(""); }} autoComplete="current-password" placeholder="Votre mot de passe créé" className="pt-6 pb-5 px-5 bg-gray-800/50 border-gray-600 rounded-2xl text-gray-100 placeholder:text-gray-400" />
        </div>
        <Button
          className="w-full"
