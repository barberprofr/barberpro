...
export interface AcomptesResponse {
  acomptes: Acompte[];
  total: number;
...
export function useAcomptes(stylistId?: string, startDate?: number, endDate?: number, validated?: boolean) {
  const salonId = getSelectedSalon();
  return useQuery({
    queryKey: ['acomptes', salonId, stylistId, startDate, endDate, validated],
    queryFn: async () => {
      const params = new URLSearchParams();
      ...
      const res = await apiFetch(`/api/acomptes?${params.toString()}`);
      if (!res.ok) throw new Error('Failed to fetch acomptes');
      return res.json() as Promise<AcomptesResponse>;
    }
  });
}

export function useStylistAcomptes(stylistId?: string, startDate?: number, endDate?: number) {
  const salonId = getSelectedSalon();
  return useQuery({
    queryKey: ['stylist-acomptes', salonId, stylistId, startDate, endDate],
    enabled: !!stylistId,
    queryFn: async () => {
      const params = new URLSearchParams();
      if (startDate) params.append('startDate', startDate.toString());
      ...
      const res = await apiFetch(`/api/stylists/${stylistId}/acomptes?${params.toString()}`);
      if (!res.ok) throw new Error('Failed to fetch stylist acomptes');
      return res.json() as Promise<{ acomptes: Acompte[]; totalValidated: number; count: number }>;
    }
  });
}
...
export function useCreateAcompte() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: { stylistId: string; amount: number; note?: string }) => {
      const res = await apiFetch('/api/acomptes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(input),
      });
      if (!res.ok) await throwResponseError(res);
      return res.json() as Promise<{ acompte: Acompte }>;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['acomptes'] });
      qc.invalidateQueries({ queryKey: ['stylist-acomptes'] });
    }
  });
}

export function useValidateAcompte() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => {
      const res = await apiFetch(`/api/acomptes/${id}/validate`, {
        method: 'PATCH',
      });
      if (!res.ok) await throwResponseError(res);
      return res.json() as Promise<{ acompte: Acompte }>;
...
export function useDeleteAcompte() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => {
      const res = await apiFetch(`/api/acomptes/${id}`, {
        method: 'DELETE',
      });
      if (!res.ok) await throwResponseError(res);
      return res.json() as Promise<{ ok: true }>;
